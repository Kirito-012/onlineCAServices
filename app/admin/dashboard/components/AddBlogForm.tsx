...
export default function AddBlogForm({onBlogAdded}: AddBlogFormProps) {
	...
	const handleSubmit = async (e: FormEvent) => {
		...
		try {
			...
			if (response.ok) {
				toast.success('Successfully Added to the Blog')
				// Reset form
				setMetaTitle('')
				setMetaDescription('')
				setImage('')
		...
		<form
			...
			<div className='flex gap-4 pt-6 border-t border-gray-200'>
				...
				<button
					...
					onClick={handleReset}
					disabled={loading}
					className='px-8 py-3.5 border-2 cursor-pointer border-gray-300 text-gray-700 rounded-lg font-semibold hover:bg-gray-100 hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed'>
					Clear Form
				</button>
			</div>
		</form>
